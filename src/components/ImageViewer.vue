...
export default {
  ...
  mounted() {
    ...
    this.image.onload = () => { // 読み込み完了時の処理を指定しておく
  ...
  methods: {
    ...
    drawPicture() {
      ...
      const canvas = document.getElementById("canvas")
      const context = canvas.getContext("2d")
      context.drawImage(this.image, 0, 0, this.imgWidth, this.imgHeight, 0, (this.canvasHeight - this.actualImgHeight) / 2, this.actualImgWidth, this.actualImgHeight)
    },
    drawRuler() {
    ...
    updateCanvasSize() {
      ...
      console.log(`キャンバス 幅:${this.canvasWidth}, 高さ:${this.canvasHeight}`)
    }
  }
}
</script>
